...
import { updateFacility } from '../../../../apis/settingsActions';

const EditFacility = ({ setFacilityEditModal, facility }) => {

    const { token, refreshRecord } = useContext(AppContext);
    const [facility_name, setFacility_name] = useState(facility?.facility_name);
    ...
    const handleSubmit = (e) => {
        ...
        const data = {
            id: facility?.facility_id, facility_name, lga_id
        }

        updateFacility(token, data, setSuccess, setError, setSaving)
    }
                    ...
                    <div className={`w-full md:w-[500px] bg-gray-100 dark:bg-gray-800 px-6 py-1 rounded-md`}>
                        <div className='flex justify-between items-center border-b border-gray-200 dark:border-gray-700 py-2'>
                            ...
                            <span
                                ...
                                onClick={() => setFacilityEditModal(false)}
                            >    
                        ...
                        <div className='w-full my-4'>
                            <form onSubmit={handleSubmit} className='space-y-4'>
                            {
                                ...
                                <StateOptions setState_id={setState_id} state_id={facility?.state_id} />
                                <LgaOptions setLga_id={setLga_id} state_id={facility?.state_id} lga_id={facility?.lga_id} />
                                <input 
                                    type='text' 
                                    value={facility_name}
